...
from nats.js.api import ConsumerConfig, KeyValueConfig, PriorityPolicy, StorageType
...
class StaticConsumerGroupConsumerInstance(ConsumerGroupConsumeContext):
    ...
    async def _join_member_consumer_static(self) -> None:
        ...
        consumer_config.priority_groups = [self.member_name]
        consumer_config.priority_policy = PriorityPolicy.PINNED
        consumer_config.priority_timeout = consumer_config.ack_wait
    ...
    async def _start_consuming(self) -> None:
        ...
        # In nats-py, we use pull_subscribe with the consumer name and priority_group
...
async def static_member_step_down(
    ...
    try:
        await js.unpin_consumer(stream_name, consumer_name, member_name)
    except Exception as e:
        logger.error("Error trying to unpin the member's consumer: %s", e)
        raise
