...
class ElasticConsumerGroupConsumerInstance(ConsumerGroupConsumeContext):
    ...
    async def _join_member_consumer(self) -> None:
        ...
        # Configure priority groups for pinned consumer behavior
        config.priority_groups = [self.member_name]
        config.priority_policy = PriorityPolicy.PINNED
        config.priority_timeout = config.ack_wait
        ...
        try:
            ...
            if "consumer already exists" in str(e).lower():
                ...
                try:
                    ...
                    # Will retry after timeout in instance routine
                    logger.debug(
                        "Failed to delete and recreate consumer, will retry later"
                    )
                    return
            else:
                # Some other API error - will retry after timeout
                logger.debug("API error creating consumer, will retry later: %s", e)
                return
        except Exception as e:  # noqa: BLE001
            # Not logging at info level because errors can happen during normal operation
            # (e.g., "filtered consumer not unique on workqueue stream" can occur when
            # members aren't perfectly synchronized processing membership changes)
            logger.debug("Error creating consumer, will retry later: %s", e)
            return
    ...
    async def _start_consuming(self) -> None:
        ...
        # Create pull subscription with priority_group
        self.subscription = await self.js.pull_subscribe(
            "",
            durable=self.member_name,
            stream=consumer_stream_name,
            priority_group=self.member_name,
        )
        ...
        async def process_messages():
            while not self._cancel_event.is_set():
                try:
                    ...
                    # Continue trying to fetch messages, don't break on transient errors
                    continue
    ...
    async def _stop_consuming(self) -> None:
        ...
        if self.subscription:
            try:
                ...
                # Ignore errors during cleanup - subscription may already be closed
                logger.debug("Error unsubscribing during stop: %s", e)
            self.subscription = None
    ...
    async def _process_membership_change(self) -> None:
        ...
        if self.consumer:
            ...
            try:
                ...
                # Check if we're the pinned consumer by examining priority_groups
                if (
                    hasattr(consumer_info, "priority_groups")
                    and consumer_info.priority_groups
                ):
                    for pg in consumer_info.priority_groups:
                        if (
                            hasattr(pg, "group")
                            and pg.group == self.member_name
                            and hasattr(pg, "pinned_client_id")
                            and pg.pinned_client_id == self.current_pinned_id
                        ):
                            is_pinned = True
                            break
            except Exception as e:  # noqa: BLE001
                # Ignore errors getting consumer info - consumer may not exist yet
                logger.debug("Error getting consumer info: %s", e)
...
async def elastic_member_step_down(
    ...
    # Use the unpin_consumer API to force the current pinned instance to step down
    try:
        await js.unpin_consumer(consumer_stream_name, member_name, member_name)
    except NotFoundError:
        pass
